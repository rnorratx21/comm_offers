...
class GaVisitors
  ...
  def all_time_offer_visitors_count(offer, permalink_path)
    opts = {}
    ...
    count += result_old_url.first.visitors.to_i if result_old_url.any?

    result_new_url = ga_results(opts.merge(:filters => {:page_path.eql => permalink_path})).to_a
    # puts "new url: #{result_new_url.inspect}"
    count += result_new_url.first.visitors.to_i if result_new_url.any?
